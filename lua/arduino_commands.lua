...
M.board_data = { fqnb = nil, core = nil, port = nil }
...
function M.board_data_detection()
  M.read_card_data() -- Attempt to load existing settings first
  ...
  for line in output:gmatch('[^\n]+') do
    if line:match('[%w_]+:[%w_]+:') then
      ...
      M.board_data.fqnb = M.board_data.fqnb or line:match('([%w_]+:[%w_]+:[%w_]+)')
